...
@implementation INSQueryViewController (Feed)

- (void)clickFeedForwardFrom:(INSFeed *)feed {
    [self ins_openLinkInSafari:feed.forwardFrom];
}

- (void)commentFeed:(INSFeed *)feed {
    INSCommentQueryViewModel *commentQueryVM = [[INSCommentQueryViewModel alloc] initWithFeed:feed];
    INSCommentQueryViewController *commentQyeryVC = [[INSCommentQueryViewController alloc] initWithQueryVM:commentQueryVM];
